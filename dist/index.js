...
// Tool: search_skills
server.tool("search_skills", "Search for AI agent skills on agentskill.sh. Returns matching skills with name, description, rating, and install count.", {
    ...
    const params = new URLSearchParams({
        ...
        fields: "name,slug,description,owner,platforms,installCount,score,ratingCount,repositoryUrl",
    });
    if (platform)
...
// Tool: get_skill
server.tool("get_skill", "Get full details for a specific skill including its SKILL.md content, security info, and metadata.", {
    slug: z
        .string()
    ...
    if (!s) {
        return {
            content: [{ type: "text", text: `Skill "${slug}" not found.` }],
        };
    }
    const rating = s.score
        ? `${s.score.toFixed(1)}/5 (${s.ratingCount} ratings)`
    ...
    sections.push("", `Install: use the install_skill tool with slug "${s.slug}"`, `View on web: https://agentskill.sh/skills/${s.slug}`);
    return {
        content: [{ type: "text", text: sections.join("\n") }],
    };
});
// Tool: install_skill
server.tool("install_skill", "Install a skill from agentskill.sh to the local skills directory. Downloads the SKILL.md file and any associated files.", {
...
// Tool: get_trending
server.tool("get_trending", "Get trending and popular skills on agentskill.sh.", {
    period: z
        .enum(["hot", "trending", "top"])
        .optional()
        .describe("'hot' for 24h, 'trending' for 7 days, 'top' for all time (default: trending)"),
    platform: z
        .string()
        .optional()
    ...
    if (!data.data?.length) {
        return {
            content: [
                { type: "text", text: "No trending skills found." },
            ],
        };
    }
    const label = section === "hot"
        ? "Hot (24h)"
        : section === "top"
            ? "Top (all time)"
            : "Trending (7 days)";
